import React,{useState} from 'react'
import Form from './Form';


function Customer() {
  const [form, setForm] = useState(false);

  const btnForm = () => {
    setForm((form) => !form);
    <Form/>;
    console.log(form);
  };
  return(
  <div
  className="animate in-view is-animated"
  data-delay={0}
  data-duration={400}
  data-animation="fade-up"
>
  <div className="section section--gray section--testimonials">
    <div className="shell">
      <div className="section__head">
        <h6>Still not convinced?</h6>
        <h2>Here’s what our customers say</h2>
      </div>
    </div>
    <div className="embla embla--alt slider-testimonial-with-video">
      <div className="slider__container embla__container is-draggable">
        <div
          className="slider__slides embla__slides"
          style={{ transform: "translate3d(26.325%, 0px, 0px)" }}
        >
          <div
            className="slider__slide embla__slide is-current-slide is-selected"
            style={{ left: "0%" }}
          >
            <div className="testimonial-with-video testimonial-with-video--yellow">
              <p>
                “I use Twicsy every single day and I love it. It's a great
                tool for my Instagram account.”
              </p>
              <div className="testimonial__video">
                <svg
                  className="svg-icon svg-icon--testimonialCheck"
                  width={35}
                  height={35}
                  viewBox="0 0 35 35"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <ellipse
                    cx="17.4999"
                    cy="17.8888"
                    rx="12.0556"
                    ry="11.6667"
                    fill="#FF553C"
                  />
                  <path
                    d="M32.5925 12.5748C31.8934 11.9859 31.5441 11.0803 31.6676 10.1744C31.8607 8.3074 31.2022 6.45242 29.8747 5.12528C28.5476 3.79776 26.6926 3.13925 24.8256 3.3324C23.9197 3.45592 23.0141 3.10657 22.4252 2.40749C21.2428 0.888439 19.4254 0 17.5 0C15.5746 0 13.7572 0.888449 12.5748 2.40749C11.9859 3.10657 11.0803 3.45594 10.1744 3.3324C8.30739 3.13931 6.45241 3.79783 5.12528 5.12528C3.79776 6.45242 3.13924 8.3074 3.3324 10.1744C3.45592 11.0803 3.10657 11.9859 2.40749 12.5748C0.888439 13.7572 0 15.5746 0 17.5C0 19.4254 0.888448 21.2428 2.40749 22.4252C3.10657 23.0141 3.45594 23.9197 3.3324 24.8256C3.13931 26.6926 3.79783 28.5476 5.12528 29.8747C6.45241 31.2022 8.30739 31.8608 10.1744 31.6676C11.0803 31.5441 11.9859 31.8934 12.5748 32.5925C13.7572 34.1116 15.5746 35 17.5 35C19.4254 35 21.2428 34.1116 22.4252 32.5925C23.0141 31.8934 23.9197 31.5441 24.8256 31.6676C26.6926 31.8607 28.5476 31.2022 29.8747 29.8747C31.2022 28.5476 31.8608 26.6926 31.6676 24.8256C31.5441 23.9197 31.8934 23.0141 32.5925 22.4252C34.1116 21.2428 35 19.4254 35 17.5C35 15.5746 34.1116 13.7572 32.5925 12.5748V12.5748ZM27.4505 15.9084L18.2832 25.0756H18.2836C17.8144 25.5463 17.1771 25.8108 16.5126 25.8108C15.8477 25.8108 15.2108 25.5463 14.7416 25.0756L9.34135 19.6668H9.34098C8.71445 19.028 8.47597 18.1039 8.71556 17.2414C8.95553 16.3794 9.63676 15.7108 10.5033 15.4872C11.3698 15.264 12.2895 15.5196 12.9164 16.1584L16.5498 19.8003L23.9506 12.3996C24.5816 11.7671 25.5024 11.5189 26.3659 11.7492C27.2294 11.9795 27.9046 12.6533 28.1368 13.5161C28.3694 14.3793 28.1234 15.3004 27.4925 15.9333L27.4505 15.9084Z"
                    fill="#FFB6AB"
                  />
                </svg>
                <div className="video-holder is-animated">
                  <video
                    src="/videos/get-followers-video-1.mp4"
                    autoPlay=""
                    loop=""
                    playsInline=""
                  />
                </div>
              </div>
              <i className="svg-blob svg-blob--BlobSmiling testimonial__blob">
                <svg
                  width={227}
                  height={252}
                  viewBox="0 0 227 252"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M30.8171 236.433C11.2663 220.138 0.0139014 215.924 0.0793455 191.422L0.462322 75.6222C0.579858 31.6101 36.9051 -1.86746 81.5971 0.848022C126.289 3.56352 162.424 41.4438 162.306 85.456L162.195 99.4671L163.279 243.722L99.9058 231.551C75.736 235.963 50.3678 252.728 30.8171 236.433Z"
                    fill="url(#paint0_linear_0_1)"
                  />
                  <path
                    d="M30.8171 236.433C11.2663 220.138 0.0139014 215.924 0.0793455 191.422L0.462322 75.6222C0.579858 31.6101 36.9051 -1.86746 81.5971 0.848022C126.289 3.56352 162.424 41.4438 162.306 85.456L162.195 99.4671L163.279 243.722L99.9058 231.551C75.736 235.963 50.3678 252.728 30.8171 236.433Z"
                    fill="#FFCE54"
                  />
                  <path
                    opacity="0.6"
                    d="M60.0242 30.1465C63.523 38.5012 73.1092 37.5705 76.1684 31.084M84.3365 31.084C87.8353 39.4387 97.4214 38.508 100.481 32.0214M43.8801 36.6044C60.0242 57.124 104.517 57.2282 117.49 37.6461"
                    stroke="#A87800"
                    strokeWidth="3.90705"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
              </i>
            </div>
          </div>
          <div
            className="slider__slide embla__slide is-next-slide is-selected"
            style={{ left: "0%" }}
          >
            <div className="testimonial-with-video testimonial-with-video--cyan">
              <p>
                “I've been using Twicsy for a few months now and it's really
                great. Best decision I've ever made.”
              </p>
              <div className="testimonial__video">
                <svg
                  className="svg-icon svg-icon--testimonialCheck"
                  width={35}
                  height={35}
                  viewBox="0 0 35 35"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <ellipse
                    cx="17.4999"
                    cy="17.8888"
                    rx="12.0556"
                    ry="11.6667"
                    fill="#FF553C"
                  />
                  <path
                    d="M32.5925 12.5748C31.8934 11.9859 31.5441 11.0803 31.6676 10.1744C31.8607 8.3074 31.2022 6.45242 29.8747 5.12528C28.5476 3.79776 26.6926 3.13925 24.8256 3.3324C23.9197 3.45592 23.0141 3.10657 22.4252 2.40749C21.2428 0.888439 19.4254 0 17.5 0C15.5746 0 13.7572 0.888449 12.5748 2.40749C11.9859 3.10657 11.0803 3.45594 10.1744 3.3324C8.30739 3.13931 6.45241 3.79783 5.12528 5.12528C3.79776 6.45242 3.13924 8.3074 3.3324 10.1744C3.45592 11.0803 3.10657 11.9859 2.40749 12.5748C0.888439 13.7572 0 15.5746 0 17.5C0 19.4254 0.888448 21.2428 2.40749 22.4252C3.10657 23.0141 3.45594 23.9197 3.3324 24.8256C3.13931 26.6926 3.79783 28.5476 5.12528 29.8747C6.45241 31.2022 8.30739 31.8608 10.1744 31.6676C11.0803 31.5441 11.9859 31.8934 12.5748 32.5925C13.7572 34.1116 15.5746 35 17.5 35C19.4254 35 21.2428 34.1116 22.4252 32.5925C23.0141 31.8934 23.9197 31.5441 24.8256 31.6676C26.6926 31.8607 28.5476 31.2022 29.8747 29.8747C31.2022 28.5476 31.8608 26.6926 31.6676 24.8256C31.5441 23.9197 31.8934 23.0141 32.5925 22.4252C34.1116 21.2428 35 19.4254 35 17.5C35 15.5746 34.1116 13.7572 32.5925 12.5748V12.5748ZM27.4505 15.9084L18.2832 25.0756H18.2836C17.8144 25.5463 17.1771 25.8108 16.5126 25.8108C15.8477 25.8108 15.2108 25.5463 14.7416 25.0756L9.34135 19.6668H9.34098C8.71445 19.028 8.47597 18.1039 8.71556 17.2414C8.95553 16.3794 9.63676 15.7108 10.5033 15.4872C11.3698 15.264 12.2895 15.5196 12.9164 16.1584L16.5498 19.8003L23.9506 12.3996C24.5816 11.7671 25.5024 11.5189 26.3659 11.7492C27.2294 11.9795 27.9046 12.6533 28.1368 13.5161C28.3694 14.3793 28.1234 15.3004 27.4925 15.9333L27.4505 15.9084Z"
                    fill="#FFB6AB"
                  />
                </svg>
                <div className="video-holder">
                  <video src="" autoPlay="" loop="" playsInline="" />
                </div>
              </div>
              <i className="svg-blob svg-blob--BlobSmiling testimonial__blob">
                <svg
                  width={227}
                  height={252}
                  viewBox="0 0 227 252"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M30.8171 236.433C11.2663 220.138 0.0139014 215.924 0.0793455 191.422L0.462322 75.6222C0.579858 31.6101 36.9051 -1.86746 81.5971 0.848022C126.289 3.56352 162.424 41.4438 162.306 85.456L162.195 99.4671L163.279 243.722L99.9058 231.551C75.736 235.963 50.3678 252.728 30.8171 236.433Z"
                    fill="url(#paint0_linear_0_1)"
                  />
                  <path
                    d="M30.8171 236.433C11.2663 220.138 0.0139014 215.924 0.0793455 191.422L0.462322 75.6222C0.579858 31.6101 36.9051 -1.86746 81.5971 0.848022C126.289 3.56352 162.424 41.4438 162.306 85.456L162.195 99.4671L163.279 243.722L99.9058 231.551C75.736 235.963 50.3678 252.728 30.8171 236.433Z"
                    fill="#91EAF0"
                  />
                  <path
                    opacity="0.6"
                    d="M60.0242 30.1465C63.523 38.5012 73.1092 37.5705 76.1684 31.084M84.3365 31.084C87.8353 39.4387 97.4214 38.508 100.481 32.0214M43.8801 36.6044C60.0242 57.124 104.517 57.2282 117.49 37.6461"
                    stroke="#0C7C84"
                    strokeWidth="3.90705"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
              </i>
            </div>
          </div>
          <div className="slider__slide embla__slide">
            <div className="testimonial-with-video testimonial-with-video--purple">
              <p>
                “Not only is it easy to use, but it's also very powerful. I've
                been consistently hitting the explore page for the past few
                weeks.”
              </p>
              <div className="testimonial__video">
                <svg
                  className="svg-icon svg-icon--testimonialCheck"
                  width={35}
                  height={35}
                  viewBox="0 0 35 35"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <ellipse
                    cx="17.4999"
                    cy="17.8888"
                    rx="12.0556"
                    ry="11.6667"
                    fill="#FF553C"
                  />
                  <path
                    d="M32.5925 12.5748C31.8934 11.9859 31.5441 11.0803 31.6676 10.1744C31.8607 8.3074 31.2022 6.45242 29.8747 5.12528C28.5476 3.79776 26.6926 3.13925 24.8256 3.3324C23.9197 3.45592 23.0141 3.10657 22.4252 2.40749C21.2428 0.888439 19.4254 0 17.5 0C15.5746 0 13.7572 0.888449 12.5748 2.40749C11.9859 3.10657 11.0803 3.45594 10.1744 3.3324C8.30739 3.13931 6.45241 3.79783 5.12528 5.12528C3.79776 6.45242 3.13924 8.3074 3.3324 10.1744C3.45592 11.0803 3.10657 11.9859 2.40749 12.5748C0.888439 13.7572 0 15.5746 0 17.5C0 19.4254 0.888448 21.2428 2.40749 22.4252C3.10657 23.0141 3.45594 23.9197 3.3324 24.8256C3.13931 26.6926 3.79783 28.5476 5.12528 29.8747C6.45241 31.2022 8.30739 31.8608 10.1744 31.6676C11.0803 31.5441 11.9859 31.8934 12.5748 32.5925C13.7572 34.1116 15.5746 35 17.5 35C19.4254 35 21.2428 34.1116 22.4252 32.5925C23.0141 31.8934 23.9197 31.5441 24.8256 31.6676C26.6926 31.8607 28.5476 31.2022 29.8747 29.8747C31.2022 28.5476 31.8608 26.6926 31.6676 24.8256C31.5441 23.9197 31.8934 23.0141 32.5925 22.4252C34.1116 21.2428 35 19.4254 35 17.5C35 15.5746 34.1116 13.7572 32.5925 12.5748V12.5748ZM27.4505 15.9084L18.2832 25.0756H18.2836C17.8144 25.5463 17.1771 25.8108 16.5126 25.8108C15.8477 25.8108 15.2108 25.5463 14.7416 25.0756L9.34135 19.6668H9.34098C8.71445 19.028 8.47597 18.1039 8.71556 17.2414C8.95553 16.3794 9.63676 15.7108 10.5033 15.4872C11.3698 15.264 12.2895 15.5196 12.9164 16.1584L16.5498 19.8003L23.9506 12.3996C24.5816 11.7671 25.5024 11.5189 26.3659 11.7492C27.2294 11.9795 27.9046 12.6533 28.1368 13.5161C28.3694 14.3793 28.1234 15.3004 27.4925 15.9333L27.4505 15.9084Z"
                    fill="#FFB6AB"
                  />
                </svg>
                <div className="video-holder">
                  <video src="" autoPlay="" loop="" playsInline="" />
                </div>
              </div>
              <i className="svg-blob svg-blob--BlobSmilingAlt testimonial__blob">
                <svg
                  width={280}
                  height={370}
                  viewBox="0 0 280 370"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g opacity="0.4">
                    <path
                      d="M109.817 268.433C90.2663 252.138 79.0139 247.924 79.0793 223.422L79.4623 107.622C79.5799 63.61 115.905 30.1325 160.597 32.848C205.289 35.5635 241.424 73.4438 241.306 117.456L241.195 131.467L242.279 275.722L178.906 263.551C154.736 267.963 129.368 284.728 109.817 268.433Z"
                      fill="url(#paint0_linear_497_3813)"
                    />
                    <path
                      d="M109.817 268.433C90.2663 252.138 79.0139 247.924 79.0793 223.422L79.4623 107.622C79.5799 63.61 115.905 30.1325 160.597 32.848C205.289 35.5635 241.424 73.4438 241.306 117.456L241.195 131.467L242.279 275.722L178.906 263.551C154.736 267.963 129.368 284.728 109.817 268.433Z"
                      fill="#D5B5FF"
                    />
                  </g>
                  <path
                    d="M136.505 63.5987C140.439 72.9941 151.219 71.9474 154.66 64.6529M163.845 64.6529C167.78 74.0483 178.56 73.0016 182 65.7071"
                    stroke="#6635A6"
                    strokeWidth="4.39369"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M144.349 81.4268C150.442 95.4399 167.136 93.8789 172.463 82.9992"
                    stroke="#6635A6"
                    strokeWidth="4.39369"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
              </i>
            </div>
          </div>
          <div className="slider__slide embla__slide">
            <div className="testimonial-with-video testimonial-with-video--yellow">
              <p>
                “Our company has been skyrocketed by using Twicsy. I wish we
                knew about it sooner.”
              </p>
              <div className="testimonial__video">
                <svg
                  className="svg-icon svg-icon--testimonialCheck"
                  width={35}
                  height={35}
                  viewBox="0 0 35 35"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <ellipse
                    cx="17.4999"
                    cy="17.8888"
                    rx="12.0556"
                    ry="11.6667"
                    fill="#FF553C"
                  />
                  <path
                    d="M32.5925 12.5748C31.8934 11.9859 31.5441 11.0803 31.6676 10.1744C31.8607 8.3074 31.2022 6.45242 29.8747 5.12528C28.5476 3.79776 26.6926 3.13925 24.8256 3.3324C23.9197 3.45592 23.0141 3.10657 22.4252 2.40749C21.2428 0.888439 19.4254 0 17.5 0C15.5746 0 13.7572 0.888449 12.5748 2.40749C11.9859 3.10657 11.0803 3.45594 10.1744 3.3324C8.30739 3.13931 6.45241 3.79783 5.12528 5.12528C3.79776 6.45242 3.13924 8.3074 3.3324 10.1744C3.45592 11.0803 3.10657 11.9859 2.40749 12.5748C0.888439 13.7572 0 15.5746 0 17.5C0 19.4254 0.888448 21.2428 2.40749 22.4252C3.10657 23.0141 3.45594 23.9197 3.3324 24.8256C3.13931 26.6926 3.79783 28.5476 5.12528 29.8747C6.45241 31.2022 8.30739 31.8608 10.1744 31.6676C11.0803 31.5441 11.9859 31.8934 12.5748 32.5925C13.7572 34.1116 15.5746 35 17.5 35C19.4254 35 21.2428 34.1116 22.4252 32.5925C23.0141 31.8934 23.9197 31.5441 24.8256 31.6676C26.6926 31.8607 28.5476 31.2022 29.8747 29.8747C31.2022 28.5476 31.8608 26.6926 31.6676 24.8256C31.5441 23.9197 31.8934 23.0141 32.5925 22.4252C34.1116 21.2428 35 19.4254 35 17.5C35 15.5746 34.1116 13.7572 32.5925 12.5748V12.5748ZM27.4505 15.9084L18.2832 25.0756H18.2836C17.8144 25.5463 17.1771 25.8108 16.5126 25.8108C15.8477 25.8108 15.2108 25.5463 14.7416 25.0756L9.34135 19.6668H9.34098C8.71445 19.028 8.47597 18.1039 8.71556 17.2414C8.95553 16.3794 9.63676 15.7108 10.5033 15.4872C11.3698 15.264 12.2895 15.5196 12.9164 16.1584L16.5498 19.8003L23.9506 12.3996C24.5816 11.7671 25.5024 11.5189 26.3659 11.7492C27.2294 11.9795 27.9046 12.6533 28.1368 13.5161C28.3694 14.3793 28.1234 15.3004 27.4925 15.9333L27.4505 15.9084Z"
                    fill="#FFB6AB"
                  />
                </svg>
                <div className="video-holder">
                  <video src="" autoPlay="" loop="" playsInline="" />
                </div>
              </div>
              <i className="svg-blob svg-blob--BlobSmiling testimonial__blob">
                <svg
                  width={227}
                  height={252}
                  viewBox="0 0 227 252"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M30.8171 236.433C11.2663 220.138 0.0139014 215.924 0.0793455 191.422L0.462322 75.6222C0.579858 31.6101 36.9051 -1.86746 81.5971 0.848022C126.289 3.56352 162.424 41.4438 162.306 85.456L162.195 99.4671L163.279 243.722L99.9058 231.551C75.736 235.963 50.3678 252.728 30.8171 236.433Z"
                    fill="url(#paint0_linear_0_1)"
                  />
                  <path
                    d="M30.8171 236.433C11.2663 220.138 0.0139014 215.924 0.0793455 191.422L0.462322 75.6222C0.579858 31.6101 36.9051 -1.86746 81.5971 0.848022C126.289 3.56352 162.424 41.4438 162.306 85.456L162.195 99.4671L163.279 243.722L99.9058 231.551C75.736 235.963 50.3678 252.728 30.8171 236.433Z"
                    fill="#FFCE54"
                  />
                  <path
                    opacity="0.6"
                    d="M60.0242 30.1465C63.523 38.5012 73.1092 37.5705 76.1684 31.084M84.3365 31.084C87.8353 39.4387 97.4214 38.508 100.481 32.0214M43.8801 36.6044C60.0242 57.124 104.517 57.2282 117.49 37.6461"
                    stroke="#A87800"
                    strokeWidth="3.90705"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
              </i>
            </div>
          </div>
          <div className="slider__slide embla__slide">
            <div className="testimonial-with-video testimonial-with-video--cyan">
              <p>
                “Sometimes things might be too good to be true. But with
                Twicsy, that's not the case.”
              </p>
              <div className="testimonial__video">
                <svg
                  className="svg-icon svg-icon--testimonialCheck"
                  width={35}
                  height={35}
                  viewBox="0 0 35 35"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <ellipse
                    cx="17.4999"
                    cy="17.8888"
                    rx="12.0556"
                    ry="11.6667"
                    fill="#FF553C"
                  />
                  <path
                    d="M32.5925 12.5748C31.8934 11.9859 31.5441 11.0803 31.6676 10.1744C31.8607 8.3074 31.2022 6.45242 29.8747 5.12528C28.5476 3.79776 26.6926 3.13925 24.8256 3.3324C23.9197 3.45592 23.0141 3.10657 22.4252 2.40749C21.2428 0.888439 19.4254 0 17.5 0C15.5746 0 13.7572 0.888449 12.5748 2.40749C11.9859 3.10657 11.0803 3.45594 10.1744 3.3324C8.30739 3.13931 6.45241 3.79783 5.12528 5.12528C3.79776 6.45242 3.13924 8.3074 3.3324 10.1744C3.45592 11.0803 3.10657 11.9859 2.40749 12.5748C0.888439 13.7572 0 15.5746 0 17.5C0 19.4254 0.888448 21.2428 2.40749 22.4252C3.10657 23.0141 3.45594 23.9197 3.3324 24.8256C3.13931 26.6926 3.79783 28.5476 5.12528 29.8747C6.45241 31.2022 8.30739 31.8608 10.1744 31.6676C11.0803 31.5441 11.9859 31.8934 12.5748 32.5925C13.7572 34.1116 15.5746 35 17.5 35C19.4254 35 21.2428 34.1116 22.4252 32.5925C23.0141 31.8934 23.9197 31.5441 24.8256 31.6676C26.6926 31.8607 28.5476 31.2022 29.8747 29.8747C31.2022 28.5476 31.8608 26.6926 31.6676 24.8256C31.5441 23.9197 31.8934 23.0141 32.5925 22.4252C34.1116 21.2428 35 19.4254 35 17.5C35 15.5746 34.1116 13.7572 32.5925 12.5748V12.5748ZM27.4505 15.9084L18.2832 25.0756H18.2836C17.8144 25.5463 17.1771 25.8108 16.5126 25.8108C15.8477 25.8108 15.2108 25.5463 14.7416 25.0756L9.34135 19.6668H9.34098C8.71445 19.028 8.47597 18.1039 8.71556 17.2414C8.95553 16.3794 9.63676 15.7108 10.5033 15.4872C11.3698 15.264 12.2895 15.5196 12.9164 16.1584L16.5498 19.8003L23.9506 12.3996C24.5816 11.7671 25.5024 11.5189 26.3659 11.7492C27.2294 11.9795 27.9046 12.6533 28.1368 13.5161C28.3694 14.3793 28.1234 15.3004 27.4925 15.9333L27.4505 15.9084Z"
                    fill="#FFB6AB"
                  />
                </svg>
                <div className="video-holder">
                  <video src="" autoPlay="" loop="" playsInline="" />
                </div>
              </div>
              <i className="svg-blob svg-blob--BlobSmiling testimonial__blob">
                <svg
                  width={227}
                  height={252}
                  viewBox="0 0 227 252"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M30.8171 236.433C11.2663 220.138 0.0139014 215.924 0.0793455 191.422L0.462322 75.6222C0.579858 31.6101 36.9051 -1.86746 81.5971 0.848022C126.289 3.56352 162.424 41.4438 162.306 85.456L162.195 99.4671L163.279 243.722L99.9058 231.551C75.736 235.963 50.3678 252.728 30.8171 236.433Z"
                    fill="url(#paint0_linear_0_1)"
                  />
                  <path
                    d="M30.8171 236.433C11.2663 220.138 0.0139014 215.924 0.0793455 191.422L0.462322 75.6222C0.579858 31.6101 36.9051 -1.86746 81.5971 0.848022C126.289 3.56352 162.424 41.4438 162.306 85.456L162.195 99.4671L163.279 243.722L99.9058 231.551C75.736 235.963 50.3678 252.728 30.8171 236.433Z"
                    fill="#91EAF0"
                  />
                  <path
                    opacity="0.6"
                    d="M60.0242 30.1465C63.523 38.5012 73.1092 37.5705 76.1684 31.084M84.3365 31.084C87.8353 39.4387 97.4214 38.508 100.481 32.0214M43.8801 36.6044C60.0242 57.124 104.517 57.2282 117.49 37.6461"
                    stroke="#0C7C84"
                    strokeWidth="3.90705"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
              </i>
            </div>
          </div>
          <div
            className="slider__slide embla__slide is-selected"
            style={{ left: "-284.1%" }}
          >
            <div className="testimonial-with-video testimonial-with-video--purple">
              <p>
                “Really love the simplicity of the interface and amazing
                customer support.”
              </p>
              <div className="testimonial__video">
                <svg
                  className="svg-icon svg-icon--testimonialCheck"
                  width={35}
                  height={35}
                  viewBox="0 0 35 35"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <ellipse
                    cx="17.4999"
                    cy="17.8888"
                    rx="12.0556"
                    ry="11.6667"
                    fill="#FF553C"
                  />
                  <path
                    d="M32.5925 12.5748C31.8934 11.9859 31.5441 11.0803 31.6676 10.1744C31.8607 8.3074 31.2022 6.45242 29.8747 5.12528C28.5476 3.79776 26.6926 3.13925 24.8256 3.3324C23.9197 3.45592 23.0141 3.10657 22.4252 2.40749C21.2428 0.888439 19.4254 0 17.5 0C15.5746 0 13.7572 0.888449 12.5748 2.40749C11.9859 3.10657 11.0803 3.45594 10.1744 3.3324C8.30739 3.13931 6.45241 3.79783 5.12528 5.12528C3.79776 6.45242 3.13924 8.3074 3.3324 10.1744C3.45592 11.0803 3.10657 11.9859 2.40749 12.5748C0.888439 13.7572 0 15.5746 0 17.5C0 19.4254 0.888448 21.2428 2.40749 22.4252C3.10657 23.0141 3.45594 23.9197 3.3324 24.8256C3.13931 26.6926 3.79783 28.5476 5.12528 29.8747C6.45241 31.2022 8.30739 31.8608 10.1744 31.6676C11.0803 31.5441 11.9859 31.8934 12.5748 32.5925C13.7572 34.1116 15.5746 35 17.5 35C19.4254 35 21.2428 34.1116 22.4252 32.5925C23.0141 31.8934 23.9197 31.5441 24.8256 31.6676C26.6926 31.8607 28.5476 31.2022 29.8747 29.8747C31.2022 28.5476 31.8608 26.6926 31.6676 24.8256C31.5441 23.9197 31.8934 23.0141 32.5925 22.4252C34.1116 21.2428 35 19.4254 35 17.5C35 15.5746 34.1116 13.7572 32.5925 12.5748V12.5748ZM27.4505 15.9084L18.2832 25.0756H18.2836C17.8144 25.5463 17.1771 25.8108 16.5126 25.8108C15.8477 25.8108 15.2108 25.5463 14.7416 25.0756L9.34135 19.6668H9.34098C8.71445 19.028 8.47597 18.1039 8.71556 17.2414C8.95553 16.3794 9.63676 15.7108 10.5033 15.4872C11.3698 15.264 12.2895 15.5196 12.9164 16.1584L16.5498 19.8003L23.9506 12.3996C24.5816 11.7671 25.5024 11.5189 26.3659 11.7492C27.2294 11.9795 27.9046 12.6533 28.1368 13.5161C28.3694 14.3793 28.1234 15.3004 27.4925 15.9333L27.4505 15.9084Z"
                    fill="#FFB6AB"
                  />
                </svg>
                <div className="video-holder is-animated">
                  <video
                    src="/videos/get-followers-video-3.mp4"
                    autoPlay=""
                    loop=""
                    playsInline=""
                  />
                </div>
              </div>
              <i className="svg-blob svg-blob--BlobSmilingAlt testimonial__blob">
                <svg
                  width={280}
                  height={370}
                  viewBox="0 0 280 370"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g opacity="0.4">
                    <path
                      d="M109.817 268.433C90.2663 252.138 79.0139 247.924 79.0793 223.422L79.4623 107.622C79.5799 63.61 115.905 30.1325 160.597 32.848C205.289 35.5635 241.424 73.4438 241.306 117.456L241.195 131.467L242.279 275.722L178.906 263.551C154.736 267.963 129.368 284.728 109.817 268.433Z"
                      fill="url(#paint0_linear_497_3813)"
                    />
                    <path
                      d="M109.817 268.433C90.2663 252.138 79.0139 247.924 79.0793 223.422L79.4623 107.622C79.5799 63.61 115.905 30.1325 160.597 32.848C205.289 35.5635 241.424 73.4438 241.306 117.456L241.195 131.467L242.279 275.722L178.906 263.551C154.736 267.963 129.368 284.728 109.817 268.433Z"
                      fill="#D5B5FF"
                    />
                  </g>
                  <path
                    d="M136.505 63.5987C140.439 72.9941 151.219 71.9474 154.66 64.6529M163.845 64.6529C167.78 74.0483 178.56 73.0016 182 65.7071"
                    stroke="#6635A6"
                    strokeWidth="4.39369"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M144.349 81.4268C150.442 95.4399 167.136 93.8789 172.463 82.9992"
                    stroke="#6635A6"
                    strokeWidth="4.39369"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
              </i>
            </div>
          </div>
        </div>
      </div>
      <div className="slider__cta embla__cta">
        <button className="slider__button slider__button--prev embla__button embla__prev" />
        <button className="slider__button slider__button--next embla__button embla__next" />
      </div>
      <div className="slider__dots embla__dots">
        <button className="embla__dot is-active" />
        <button className="embla__dot" />
        <button className="embla__dot" />
        <button className="embla__dot" />
        <button className="embla__dot" />
        <button className="embla__dot" />
      </div>
    </div>
    <div className="shell">
      <div className="flex-row lg-justify-between md-justify-between sm-justify-between">
        <div className="rating">
          <div className="rating__circle">
            <span className="text-orange">5</span> <i>of 5</i>
          </div>
          <div className="rating__content">
            <h6>
              Customers rate us<strong className="text-orange">Superb</strong>
              <div className="stars text-xsm">
                <ul>
                  <li>
                    <svg
                      className="svg-icon svg-icon--star"
                      width={19}
                      height={20}
                      viewBox="0 0 19 20"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                        fill="#FF553C"
                      />
                    </svg>
                  </li>
                  <li>
                    <svg
                      className="svg-icon svg-icon--star"
                      width={19}
                      height={20}
                      viewBox="0 0 19 20"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                        fill="#FF553C"
                      />
                    </svg>
                  </li>
                  <li>
                    <svg
                      className="svg-icon svg-icon--star"
                      width={19}
                      height={20}
                      viewBox="0 0 19 20"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                        fill="#FF553C"
                      />
                    </svg>
                  </li>
                  <li>
                    <svg
                      className="svg-icon svg-icon--star"
                      width={19}
                      height={20}
                      viewBox="0 0 19 20"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                        fill="#FF553C"
                      />
                    </svg>
                  </li>
                  <li>
                    <svg
                      className="svg-icon svg-icon--star"
                      width={19}
                      height={20}
                      viewBox="0 0 19 20"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                        fill="#FF553C"
                      />
                    </svg>
                  </li>
                </ul>
              </div>
            </h6>
            <p className="text-sm">
              <strong className="text-orange">5 / 5</strong> — based on 6
              reviews
            </p>
          </div>
        </div>
        <button type="button" onClick={btnForm}
        className="btn btn--secondary hidden-xs">
          <span />
          <span />
          <span />
          <strong>Submit Your Review</strong>
        </button>
      </div>
      {form && <Form />}
    </div>
    <div className="section__inner">
      <div className="shell">
        <div className="section__footer text-sm">
          <button 
           type="button"
           onClick={btnForm}
           className="btn btn--secondary visible-xs">
            <span />
            <span />
            <span />
            <strong>{form ? <Form /> : "Submit Your Review"}</strong>
          </button>
        </div>
      </div>
      <div className="embla slider-testimonials">
        <div className="slider__container embla__container is-draggable">
          <div
            className="slider__slides embla__slides"
            style={{ transform: "translate3d(-41.0526%, 0px, 0px)" }}
          >
            <div
              className="slider__slide embla__slide"
              style={{ left: "0%" }}
            >
              <div className="testimonial text-sm">
                <div className="testimonial__quotes">
                  <svg
                    className="svg-icon svg-icon--quotes"
                    width={21}
                    height={15}
                    viewBox="0 0 21 15"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M19.8184 2.69092C19.2373 3.05615 18.6978 3.41309 18.1997 3.76172C17.7017 4.11035 17.27 4.48389 16.9048 4.88232C16.5396 5.26416 16.249 5.6875 16.0332 6.15234C15.834 6.60059 15.7344 7.10693 15.7344 7.67139C15.7344 7.854 15.751 8.02832 15.7842 8.19434C15.8174 8.36035 15.8589 8.54297 15.9087 8.74219C16.0083 8.65918 16.1992 8.56787 16.4814 8.46826C16.7637 8.36865 17.0708 8.31885 17.4028 8.31885C18.3491 8.31885 19.063 8.59277 19.5444 9.14062C20.0259 9.67187 20.2666 10.3608 20.2666 11.2075C20.2666 11.6392 20.1836 12.0625 20.0176 12.4775C19.8682 12.876 19.644 13.2329 19.3452 13.5483C19.063 13.8638 18.7144 14.1211 18.2993 14.3203C17.8843 14.5029 17.4194 14.5942 16.9048 14.5942C15.9585 14.5942 15.145 14.4365 14.4644 14.1211C13.7837 13.8057 13.2275 13.3989 12.7959 12.9009C12.3643 12.4028 12.0488 11.855 11.8496 11.2573C11.6504 10.6597 11.5508 10.0786 11.5508 9.51416C11.5508 8.30225 11.7334 7.24805 12.0986 6.35156C12.4639 5.45508 12.9536 4.6665 13.5679 3.98584C14.1987 3.30518 14.9209 2.70752 15.7344 2.19287C16.5645 1.66162 17.436 1.17188 18.3491 0.723633L19.8184 2.69092ZM8.86133 2.69092C8.28027 3.05615 7.74072 3.41309 7.24268 3.76172C6.74463 4.11035 6.31299 4.48389 5.94775 4.88232C5.58252 5.26416 5.29199 5.6875 5.07617 6.15234C4.87695 6.60059 4.77734 7.10693 4.77734 7.67139C4.77734 7.854 4.79395 8.02832 4.82715 8.19434C4.86035 8.36035 4.90186 8.54297 4.95166 8.74219C5.05127 8.65918 5.24219 8.56787 5.52441 8.46826C5.80664 8.36865 6.11377 8.31885 6.4458 8.31885C7.39209 8.31885 8.10596 8.59277 8.5874 9.14062C9.06885 9.67187 9.30957 10.3608 9.30957 11.2075C9.30957 11.6392 9.22656 12.0625 9.06055 12.4775C8.91113 12.876 8.68701 13.2329 8.38818 13.5483C8.10596 13.8638 7.75732 14.1211 7.34229 14.3203C6.92725 14.5029 6.4624 14.5942 5.94775 14.5942C5.00146 14.5942 4.18799 14.4365 3.50732 14.1211C2.82666 13.8057 2.27051 13.3989 1.83887 12.9009C1.40723 12.4028 1.0918 11.855 0.892578 11.2573C0.693359 10.6597 0.59375 10.0786 0.59375 9.51416C0.59375 8.30225 0.776367 7.24805 1.1416 6.35156C1.50684 5.45508 1.99658 4.6665 2.61084 3.98584C3.2417 3.30518 3.96387 2.70752 4.77734 2.19287C5.60742 1.66162 6.479 1.17188 7.39209 0.723633L8.86133 2.69092Z"
                      fill="#9095A0"
                    />
                  </svg>
                </div>
                <div className="stars text-xsm">
                  <ul>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                  </ul>
                </div>
                <blockquote>
                  {" "}
                  I love Instagram, great platform to share photos, videos,
                  etc., can't live a day without it 🙂 I guess I'm not the
                  only one crazy about this social network. It wasn't easy to
                  attract followers, so I decided to try this service. Thank
                  you very much, twicsy.com, for the stunning result, number
                  of my subscribers has increased significantly.{" "}
                </blockquote>
                <cite>— Matt V</cite>
              </div>
            </div>
            <div
              className="slider__slide embla__slide is-selected"
              style={{ left: "0%" }}
            >
              <div className="testimonial text-sm">
                <div className="testimonial__quotes">
                  <svg
                    className="svg-icon svg-icon--quotes"
                    width={21}
                    height={15}
                    viewBox="0 0 21 15"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M19.8184 2.69092C19.2373 3.05615 18.6978 3.41309 18.1997 3.76172C17.7017 4.11035 17.27 4.48389 16.9048 4.88232C16.5396 5.26416 16.249 5.6875 16.0332 6.15234C15.834 6.60059 15.7344 7.10693 15.7344 7.67139C15.7344 7.854 15.751 8.02832 15.7842 8.19434C15.8174 8.36035 15.8589 8.54297 15.9087 8.74219C16.0083 8.65918 16.1992 8.56787 16.4814 8.46826C16.7637 8.36865 17.0708 8.31885 17.4028 8.31885C18.3491 8.31885 19.063 8.59277 19.5444 9.14062C20.0259 9.67187 20.2666 10.3608 20.2666 11.2075C20.2666 11.6392 20.1836 12.0625 20.0176 12.4775C19.8682 12.876 19.644 13.2329 19.3452 13.5483C19.063 13.8638 18.7144 14.1211 18.2993 14.3203C17.8843 14.5029 17.4194 14.5942 16.9048 14.5942C15.9585 14.5942 15.145 14.4365 14.4644 14.1211C13.7837 13.8057 13.2275 13.3989 12.7959 12.9009C12.3643 12.4028 12.0488 11.855 11.8496 11.2573C11.6504 10.6597 11.5508 10.0786 11.5508 9.51416C11.5508 8.30225 11.7334 7.24805 12.0986 6.35156C12.4639 5.45508 12.9536 4.6665 13.5679 3.98584C14.1987 3.30518 14.9209 2.70752 15.7344 2.19287C16.5645 1.66162 17.436 1.17188 18.3491 0.723633L19.8184 2.69092ZM8.86133 2.69092C8.28027 3.05615 7.74072 3.41309 7.24268 3.76172C6.74463 4.11035 6.31299 4.48389 5.94775 4.88232C5.58252 5.26416 5.29199 5.6875 5.07617 6.15234C4.87695 6.60059 4.77734 7.10693 4.77734 7.67139C4.77734 7.854 4.79395 8.02832 4.82715 8.19434C4.86035 8.36035 4.90186 8.54297 4.95166 8.74219C5.05127 8.65918 5.24219 8.56787 5.52441 8.46826C5.80664 8.36865 6.11377 8.31885 6.4458 8.31885C7.39209 8.31885 8.10596 8.59277 8.5874 9.14062C9.06885 9.67187 9.30957 10.3608 9.30957 11.2075C9.30957 11.6392 9.22656 12.0625 9.06055 12.4775C8.91113 12.876 8.68701 13.2329 8.38818 13.5483C8.10596 13.8638 7.75732 14.1211 7.34229 14.3203C6.92725 14.5029 6.4624 14.5942 5.94775 14.5942C5.00146 14.5942 4.18799 14.4365 3.50732 14.1211C2.82666 13.8057 2.27051 13.3989 1.83887 12.9009C1.40723 12.4028 1.0918 11.855 0.892578 11.2573C0.693359 10.6597 0.59375 10.0786 0.59375 9.51416C0.59375 8.30225 0.776367 7.24805 1.1416 6.35156C1.50684 5.45508 1.99658 4.6665 2.61084 3.98584C3.2417 3.30518 3.96387 2.70752 4.77734 2.19287C5.60742 1.66162 6.479 1.17188 7.39209 0.723633L8.86133 2.69092Z"
                      fill="#9095A0"
                    />
                  </svg>
                </div>
                <div className="stars text-xsm">
                  <ul>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                  </ul>
                </div>
                <blockquote>
                  {" "}
                  This service is the best when you want to make your
                  Instagram account more popular! The price is nice. You get
                  followers and likes almost instantly. If something is wrong,
                  the support will explain everything. They will fix it so you
                  won't lose your money. I managed to increase the number of
                  subscribers from 5000 to 11000 in a day! A great result. And
                  no need to waste time doing it yourself 🙂{" "}
                </blockquote>
                <cite>— F S</cite>
              </div>
            </div>
            <div
              className="slider__slide embla__slide is-prev-slide is-selected"
              style={{ left: "0%" }}
            >
              <div className="testimonial text-sm">
                <div className="testimonial__quotes">
                  <svg
                    className="svg-icon svg-icon--quotes"
                    width={21}
                    height={15}
                    viewBox="0 0 21 15"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M19.8184 2.69092C19.2373 3.05615 18.6978 3.41309 18.1997 3.76172C17.7017 4.11035 17.27 4.48389 16.9048 4.88232C16.5396 5.26416 16.249 5.6875 16.0332 6.15234C15.834 6.60059 15.7344 7.10693 15.7344 7.67139C15.7344 7.854 15.751 8.02832 15.7842 8.19434C15.8174 8.36035 15.8589 8.54297 15.9087 8.74219C16.0083 8.65918 16.1992 8.56787 16.4814 8.46826C16.7637 8.36865 17.0708 8.31885 17.4028 8.31885C18.3491 8.31885 19.063 8.59277 19.5444 9.14062C20.0259 9.67187 20.2666 10.3608 20.2666 11.2075C20.2666 11.6392 20.1836 12.0625 20.0176 12.4775C19.8682 12.876 19.644 13.2329 19.3452 13.5483C19.063 13.8638 18.7144 14.1211 18.2993 14.3203C17.8843 14.5029 17.4194 14.5942 16.9048 14.5942C15.9585 14.5942 15.145 14.4365 14.4644 14.1211C13.7837 13.8057 13.2275 13.3989 12.7959 12.9009C12.3643 12.4028 12.0488 11.855 11.8496 11.2573C11.6504 10.6597 11.5508 10.0786 11.5508 9.51416C11.5508 8.30225 11.7334 7.24805 12.0986 6.35156C12.4639 5.45508 12.9536 4.6665 13.5679 3.98584C14.1987 3.30518 14.9209 2.70752 15.7344 2.19287C16.5645 1.66162 17.436 1.17188 18.3491 0.723633L19.8184 2.69092ZM8.86133 2.69092C8.28027 3.05615 7.74072 3.41309 7.24268 3.76172C6.74463 4.11035 6.31299 4.48389 5.94775 4.88232C5.58252 5.26416 5.29199 5.6875 5.07617 6.15234C4.87695 6.60059 4.77734 7.10693 4.77734 7.67139C4.77734 7.854 4.79395 8.02832 4.82715 8.19434C4.86035 8.36035 4.90186 8.54297 4.95166 8.74219C5.05127 8.65918 5.24219 8.56787 5.52441 8.46826C5.80664 8.36865 6.11377 8.31885 6.4458 8.31885C7.39209 8.31885 8.10596 8.59277 8.5874 9.14062C9.06885 9.67187 9.30957 10.3608 9.30957 11.2075C9.30957 11.6392 9.22656 12.0625 9.06055 12.4775C8.91113 12.876 8.68701 13.2329 8.38818 13.5483C8.10596 13.8638 7.75732 14.1211 7.34229 14.3203C6.92725 14.5029 6.4624 14.5942 5.94775 14.5942C5.00146 14.5942 4.18799 14.4365 3.50732 14.1211C2.82666 13.8057 2.27051 13.3989 1.83887 12.9009C1.40723 12.4028 1.0918 11.855 0.892578 11.2573C0.693359 10.6597 0.59375 10.0786 0.59375 9.51416C0.59375 8.30225 0.776367 7.24805 1.1416 6.35156C1.50684 5.45508 1.99658 4.6665 2.61084 3.98584C3.2417 3.30518 3.96387 2.70752 4.77734 2.19287C5.60742 1.66162 6.479 1.17188 7.39209 0.723633L8.86133 2.69092Z"
                      fill="#9095A0"
                    />
                  </svg>
                </div>
                <div className="stars text-xsm">
                  <ul>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                  </ul>
                </div>
                <blockquote>
                  {" "}
                  I used to think that on Instagram you could only gain
                  subscribers with quality content that was frequently
                  updated. But now, with the good help of this site, I am
                  constantly building up my followers. I can also increase my
                  views and likes here. And this is very important for my
                  account because with its help I conduct an educational
                  project that helps many people.{" "}
                </blockquote>
                <cite>— Sally Thomson</cite>
              </div>
            </div>
            <div className="slider__slide embla__slide is-current-slide is-selected">
              <div className="testimonial text-sm">
                <div className="testimonial__quotes">
                  <svg
                    className="svg-icon svg-icon--quotes"
                    width={21}
                    height={15}
                    viewBox="0 0 21 15"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M19.8184 2.69092C19.2373 3.05615 18.6978 3.41309 18.1997 3.76172C17.7017 4.11035 17.27 4.48389 16.9048 4.88232C16.5396 5.26416 16.249 5.6875 16.0332 6.15234C15.834 6.60059 15.7344 7.10693 15.7344 7.67139C15.7344 7.854 15.751 8.02832 15.7842 8.19434C15.8174 8.36035 15.8589 8.54297 15.9087 8.74219C16.0083 8.65918 16.1992 8.56787 16.4814 8.46826C16.7637 8.36865 17.0708 8.31885 17.4028 8.31885C18.3491 8.31885 19.063 8.59277 19.5444 9.14062C20.0259 9.67187 20.2666 10.3608 20.2666 11.2075C20.2666 11.6392 20.1836 12.0625 20.0176 12.4775C19.8682 12.876 19.644 13.2329 19.3452 13.5483C19.063 13.8638 18.7144 14.1211 18.2993 14.3203C17.8843 14.5029 17.4194 14.5942 16.9048 14.5942C15.9585 14.5942 15.145 14.4365 14.4644 14.1211C13.7837 13.8057 13.2275 13.3989 12.7959 12.9009C12.3643 12.4028 12.0488 11.855 11.8496 11.2573C11.6504 10.6597 11.5508 10.0786 11.5508 9.51416C11.5508 8.30225 11.7334 7.24805 12.0986 6.35156C12.4639 5.45508 12.9536 4.6665 13.5679 3.98584C14.1987 3.30518 14.9209 2.70752 15.7344 2.19287C16.5645 1.66162 17.436 1.17188 18.3491 0.723633L19.8184 2.69092ZM8.86133 2.69092C8.28027 3.05615 7.74072 3.41309 7.24268 3.76172C6.74463 4.11035 6.31299 4.48389 5.94775 4.88232C5.58252 5.26416 5.29199 5.6875 5.07617 6.15234C4.87695 6.60059 4.77734 7.10693 4.77734 7.67139C4.77734 7.854 4.79395 8.02832 4.82715 8.19434C4.86035 8.36035 4.90186 8.54297 4.95166 8.74219C5.05127 8.65918 5.24219 8.56787 5.52441 8.46826C5.80664 8.36865 6.11377 8.31885 6.4458 8.31885C7.39209 8.31885 8.10596 8.59277 8.5874 9.14062C9.06885 9.67187 9.30957 10.3608 9.30957 11.2075C9.30957 11.6392 9.22656 12.0625 9.06055 12.4775C8.91113 12.876 8.68701 13.2329 8.38818 13.5483C8.10596 13.8638 7.75732 14.1211 7.34229 14.3203C6.92725 14.5029 6.4624 14.5942 5.94775 14.5942C5.00146 14.5942 4.18799 14.4365 3.50732 14.1211C2.82666 13.8057 2.27051 13.3989 1.83887 12.9009C1.40723 12.4028 1.0918 11.855 0.892578 11.2573C0.693359 10.6597 0.59375 10.0786 0.59375 9.51416C0.59375 8.30225 0.776367 7.24805 1.1416 6.35156C1.50684 5.45508 1.99658 4.6665 2.61084 3.98584C3.2417 3.30518 3.96387 2.70752 4.77734 2.19287C5.60742 1.66162 6.479 1.17188 7.39209 0.723633L8.86133 2.69092Z"
                      fill="#9095A0"
                    />
                  </svg>
                </div>
                <div className="stars text-xsm">
                  <ul>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                  </ul>
                </div>
                <blockquote> Love this app! </blockquote>
                <cite>— Ellie</cite>
              </div>
            </div>
            <div
              className="slider__slide embla__slide is-next-slide is-selected"
              style={{ left: "0%" }}
            >
              <div className="testimonial text-sm">
                <div className="testimonial__quotes">
                  <svg
                    className="svg-icon svg-icon--quotes"
                    width={21}
                    height={15}
                    viewBox="0 0 21 15"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M19.8184 2.69092C19.2373 3.05615 18.6978 3.41309 18.1997 3.76172C17.7017 4.11035 17.27 4.48389 16.9048 4.88232C16.5396 5.26416 16.249 5.6875 16.0332 6.15234C15.834 6.60059 15.7344 7.10693 15.7344 7.67139C15.7344 7.854 15.751 8.02832 15.7842 8.19434C15.8174 8.36035 15.8589 8.54297 15.9087 8.74219C16.0083 8.65918 16.1992 8.56787 16.4814 8.46826C16.7637 8.36865 17.0708 8.31885 17.4028 8.31885C18.3491 8.31885 19.063 8.59277 19.5444 9.14062C20.0259 9.67187 20.2666 10.3608 20.2666 11.2075C20.2666 11.6392 20.1836 12.0625 20.0176 12.4775C19.8682 12.876 19.644 13.2329 19.3452 13.5483C19.063 13.8638 18.7144 14.1211 18.2993 14.3203C17.8843 14.5029 17.4194 14.5942 16.9048 14.5942C15.9585 14.5942 15.145 14.4365 14.4644 14.1211C13.7837 13.8057 13.2275 13.3989 12.7959 12.9009C12.3643 12.4028 12.0488 11.855 11.8496 11.2573C11.6504 10.6597 11.5508 10.0786 11.5508 9.51416C11.5508 8.30225 11.7334 7.24805 12.0986 6.35156C12.4639 5.45508 12.9536 4.6665 13.5679 3.98584C14.1987 3.30518 14.9209 2.70752 15.7344 2.19287C16.5645 1.66162 17.436 1.17188 18.3491 0.723633L19.8184 2.69092ZM8.86133 2.69092C8.28027 3.05615 7.74072 3.41309 7.24268 3.76172C6.74463 4.11035 6.31299 4.48389 5.94775 4.88232C5.58252 5.26416 5.29199 5.6875 5.07617 6.15234C4.87695 6.60059 4.77734 7.10693 4.77734 7.67139C4.77734 7.854 4.79395 8.02832 4.82715 8.19434C4.86035 8.36035 4.90186 8.54297 4.95166 8.74219C5.05127 8.65918 5.24219 8.56787 5.52441 8.46826C5.80664 8.36865 6.11377 8.31885 6.4458 8.31885C7.39209 8.31885 8.10596 8.59277 8.5874 9.14062C9.06885 9.67187 9.30957 10.3608 9.30957 11.2075C9.30957 11.6392 9.22656 12.0625 9.06055 12.4775C8.91113 12.876 8.68701 13.2329 8.38818 13.5483C8.10596 13.8638 7.75732 14.1211 7.34229 14.3203C6.92725 14.5029 6.4624 14.5942 5.94775 14.5942C5.00146 14.5942 4.18799 14.4365 3.50732 14.1211C2.82666 13.8057 2.27051 13.3989 1.83887 12.9009C1.40723 12.4028 1.0918 11.855 0.892578 11.2573C0.693359 10.6597 0.59375 10.0786 0.59375 9.51416C0.59375 8.30225 0.776367 7.24805 1.1416 6.35156C1.50684 5.45508 1.99658 4.6665 2.61084 3.98584C3.2417 3.30518 3.96387 2.70752 4.77734 2.19287C5.60742 1.66162 6.479 1.17188 7.39209 0.723633L8.86133 2.69092Z"
                      fill="#9095A0"
                    />
                  </svg>
                </div>
                <div className="stars text-xsm">
                  <ul>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                  </ul>
                </div>
                <blockquote>
                  {" "}
                  better than all other websites and this one actually works!{" "}
                </blockquote>
                <cite>— andre</cite>
              </div>
            </div>
            <div
              className="slider__slide embla__slide is-selected"
              style={{ left: "0%" }}
            >
              <div className="testimonial text-sm">
                <div className="testimonial__quotes">
                  <svg
                    className="svg-icon svg-icon--quotes"
                    width={21}
                    height={15}
                    viewBox="0 0 21 15"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M19.8184 2.69092C19.2373 3.05615 18.6978 3.41309 18.1997 3.76172C17.7017 4.11035 17.27 4.48389 16.9048 4.88232C16.5396 5.26416 16.249 5.6875 16.0332 6.15234C15.834 6.60059 15.7344 7.10693 15.7344 7.67139C15.7344 7.854 15.751 8.02832 15.7842 8.19434C15.8174 8.36035 15.8589 8.54297 15.9087 8.74219C16.0083 8.65918 16.1992 8.56787 16.4814 8.46826C16.7637 8.36865 17.0708 8.31885 17.4028 8.31885C18.3491 8.31885 19.063 8.59277 19.5444 9.14062C20.0259 9.67187 20.2666 10.3608 20.2666 11.2075C20.2666 11.6392 20.1836 12.0625 20.0176 12.4775C19.8682 12.876 19.644 13.2329 19.3452 13.5483C19.063 13.8638 18.7144 14.1211 18.2993 14.3203C17.8843 14.5029 17.4194 14.5942 16.9048 14.5942C15.9585 14.5942 15.145 14.4365 14.4644 14.1211C13.7837 13.8057 13.2275 13.3989 12.7959 12.9009C12.3643 12.4028 12.0488 11.855 11.8496 11.2573C11.6504 10.6597 11.5508 10.0786 11.5508 9.51416C11.5508 8.30225 11.7334 7.24805 12.0986 6.35156C12.4639 5.45508 12.9536 4.6665 13.5679 3.98584C14.1987 3.30518 14.9209 2.70752 15.7344 2.19287C16.5645 1.66162 17.436 1.17188 18.3491 0.723633L19.8184 2.69092ZM8.86133 2.69092C8.28027 3.05615 7.74072 3.41309 7.24268 3.76172C6.74463 4.11035 6.31299 4.48389 5.94775 4.88232C5.58252 5.26416 5.29199 5.6875 5.07617 6.15234C4.87695 6.60059 4.77734 7.10693 4.77734 7.67139C4.77734 7.854 4.79395 8.02832 4.82715 8.19434C4.86035 8.36035 4.90186 8.54297 4.95166 8.74219C5.05127 8.65918 5.24219 8.56787 5.52441 8.46826C5.80664 8.36865 6.11377 8.31885 6.4458 8.31885C7.39209 8.31885 8.10596 8.59277 8.5874 9.14062C9.06885 9.67187 9.30957 10.3608 9.30957 11.2075C9.30957 11.6392 9.22656 12.0625 9.06055 12.4775C8.91113 12.876 8.68701 13.2329 8.38818 13.5483C8.10596 13.8638 7.75732 14.1211 7.34229 14.3203C6.92725 14.5029 6.4624 14.5942 5.94775 14.5942C5.00146 14.5942 4.18799 14.4365 3.50732 14.1211C2.82666 13.8057 2.27051 13.3989 1.83887 12.9009C1.40723 12.4028 1.0918 11.855 0.892578 11.2573C0.693359 10.6597 0.59375 10.0786 0.59375 9.51416C0.59375 8.30225 0.776367 7.24805 1.1416 6.35156C1.50684 5.45508 1.99658 4.6665 2.61084 3.98584C3.2417 3.30518 3.96387 2.70752 4.77734 2.19287C5.60742 1.66162 6.479 1.17188 7.39209 0.723633L8.86133 2.69092Z"
                      fill="#9095A0"
                    />
                  </svg>
                </div>
                <div className="stars text-xsm">
                  <ul>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                    <li>
                      <svg
                        className="svg-icon svg-icon--star"
                        width={19}
                        height={20}
                        viewBox="0 0 19 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                          fill="#FF553C"
                        />
                      </svg>
                    </li>
                  </ul>
                </div>
                <blockquote>
                  {" "}
                  Great service and like that you're given a trial before
                  paying!{" "}
                </blockquote>
                <cite>— Kay</cite>
              </div>
            </div>
          </div>
        </div>
        <div className="slider__cta embla__cta">
          <button className="slider__button slider__button--prev embla__button embla__prev" />
          <button className="slider__button slider__button--next embla__button embla__next" />
        </div>
      </div>
    </div>
  </div>
</div>
  )
}

export default Customer
